...
// that same id, which is exactly what the add/build path does — the content-id
// fallback for vendored files whose filename differs from their id:
// (autobrr/harbrr#108) is exercised by whichever such defs the current snapshot
// carries, without pinning their names. A future
