...
func TestAnnounceTap(t *testing.T) {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k1"}, []*normalizer.Release{relWithGUID("A"), relWithGUID("B")})
	// 2. same key gains C: only C is new (A, B are in the prior entry + the dedup window).
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k1"}, []*normalizer.Release{relWithGUID("A"), relWithGUID("B"), relWithGUID("C")})
	// 3. a keyword search is never announced (only what a consumer already RSS-polls).
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: keyword, key: "k2"}, []*normalizer.Release{relWithGUID("D")})
	// 4. A reappears under a different RSS key: the dedup window suppresses the re-announce.
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k3"}, []*normalizer.Release{relWithGUID("A")})
...
func TestAnnounceTap_DiffsAcrossExpiry(t *testing.T) {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k"}, []*normalizer.Release{relWithGUID("A"), relWithGUID("B")})
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k"}, []*normalizer.Release{relWithGUID("A"), relWithGUID("B"), relWithGUID("C")})
...
func TestAnnounceWindow_SlidesAcrossRepeatedPriorSuppression(t *testing.T) {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k"}, []*normalizer.Release{relWithGUID("A")})
	if len(got) != 1 {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, q: empty, key: "k2"}, []*normalizer.Release{relWithGUID("A")})
	if len(got) != 1 {
...
func TestAnnounceTap_NilSinkNoPanic(t *testing.T) {
	t.Parallel()
	sc, instID, _ := testCache(t, keywordTTL, 0)
	sc.storeBestEffort(context.Background(), cacheOp{instanceID: instID, q: search.Query{}, key: "k"}, []*normalizer.Release{relWithGUID("A")})
}
