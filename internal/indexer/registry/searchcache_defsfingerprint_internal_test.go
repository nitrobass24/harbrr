...
func TestEnsureDefsFingerprints_FirstBootStoresWithoutExpiry(t *testing.T) {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: instID, cfg: map[string]string{}, q: search.Query{Keywords: "x"}, key: "k"}, relSet("A"))
...
func TestEnsureDefsFingerprints_ExpiresOnlyTheChangedDefsInstances(t *testing.T) {
	...
	ctx := context.Background()
	sc.storeBestEffort(ctx, cacheOp{instanceID: changedInst, cfg: map[string]string{}, q: search.Query{Keywords: "x"}, key: "changed"}, relSet("A"))
	sc.storeBestEffort(ctx, cacheOp{instanceID: otherInst, cfg: map[string]string{}, q: search.Query{Keywords: "y"}, key: "other"}, relSet("B"))
...
func TestEnsureDefsFingerprints_AddedAndRemovedDefs(t *testing.T) {
	...
	ctx := context.Background()
	sc.storeBestEffort(ctx, cacheOp{instanceID: fakedefInst, cfg: map[string]string{}, q: search.Query{Keywords: "x"}, key: "fakedef-row"}, relSet("A"))
	sc.storeBestEffort(ctx, cacheOp{instanceID: otherdefInst, cfg: map[string]string{}, q: search.Query{Keywords: "y"}, key: "otherdef-row"}, relSet("B"))
...
func TestEnsureDefsFingerprints_LegacyStringUpgradesWithOneFullExpire(t *testing.T) {
	...
	sc.storeBestEffort(ctx, cacheOp{instanceID: fakedefInst, cfg: map[string]string{}, q: search.Query{Keywords: "z"}, key: "after-upgrade"}, relSet("C"))
	if err := sc.EnsureDefsFingerprints(ctx, fps); err != nil {
		t.Fatalf("second EnsureDefsFingerprints: %v", err)
	}
