...
// persistOutcomes writes each outcome back to the ledger: a deleted orphan drops its
// row; everything else upserts the remote id, payload hash, and scrubbed status.
func (s *Service) persistOutcomes(ctx context.Context, connID int64, outcomes []IndexerOutcome, idBySlug map[string]int64) error {
	...
	for _, o := range outcomes {
		...
		if o.Action == ActionDeleted {
			if err := s.repo.DeleteConnectionIndexer(ctx, s.db, connID, instID); err != nil {
		...
		if err := s.repo.UpsertConnectionIndexer(ctx, s.db, row); err != nil {
