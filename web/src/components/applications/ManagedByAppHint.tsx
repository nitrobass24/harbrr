...
// own edit dialog, not this surface's PATCH. Renders nothing when appId is absent (a
// host-less download client, or a pre-migration row with no App yet) — there is no
// app to point at.
export function ManagedByAppHint({ appId }: { appId: number | null | undefined }) {
  ...
  return (
    <p className="text-[12px] text-faint">
      ...
      <a href="#apps-section" className="font-medium text-foreground underline underline-offset-2">
        {app?.name ?? `#${appId}`}
      </a>
      .
    </p>
