...
function budget(over: Partial<IndexerBudget> = {}): IndexerBudget {
  return {
    unit: "day",
    periodEnd,
    query: { used: 0, limit: 0, learned: false },
    grab: { used: 0, limit: 0, learned: false },
    ...over,
  }
}
...
describe("BudgetMeter", () => {
  it("meters a configured cap with usage and the reset boundary", () => {
    render(<BudgetMeter budget={budget({ query: { used: 1500, limit: 2000, learned: false } })} />)
  ...
  it("marks a learned cap distinctly from a configured one", () => {
    render(<BudgetMeter budget={budget({ query: { used: 40, limit: 0, learned: true } })} />)
  ...
  it("renders a spent configured budget as a held-back guard, not a failure", () => {
    render(<BudgetMeter budget={budget({ query: { used: 2000, limit: 2000, learned: false } })} />)
