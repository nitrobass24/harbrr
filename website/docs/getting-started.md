...
The fastest path is Docker. A ready-to-edit [`docker-compose.example.yml`](https://github.com/nitrobass24/harbrr/blob/main/docker-compose.example.yml)
ships in the repo; the essentials:

```yaml
services:
  harbrr:
    image: ghcr.io/nitrobass24/harbrr:latest   # see the note below on getting an image
...
- **Tag a release** — `git tag v0.1.0-alpha && git push origin v0.1.0-alpha` publishes
  `ghcr.io/nitrobass24/harbrr:0.1.0-alpha` + `:latest`.
- **PR image** (private) — `ghcr.io/nitrobass24/harbrr:pr-<n>`; `docker login ghcr.io` first.
