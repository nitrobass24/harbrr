...
| **Daemon foundation** | how the daemon stores + serves: the secrets model (`docs/security.md`), persistence, auth/session/CSRF, and where these differ from autobrr/qui | [`internal/secrets/testdata/README.md`](../internal/secrets/testdata/README.md) |
| **Live smoke** | what the live smoke + Prowlarr differential + Sonarr-orchestrated grab verified, and the auth/fetch patterns it could NOT exercise live (FlareSolverr, form login, .NET-quirk sites, cookie sites) with re-test dispositions | [`internal/smoke/README.md`](../internal/smoke/README.md) |
...
| **Native indexers** | the per-tracker native drivers for trackers Prowlarr/Jackett ship as bespoke C# (no Cardigann YAML) — the AvistaZ family plus the standalone ports (FileList, MyAnonamouse, IPTorrents, TorrentDay, BroadcastTheNet, AnimeBytes, Newznab, …) — and where each knowingly differs from its Prowlarr indexer | one `testdata/README.md` per driver under [`internal/indexer/native/`](../internal/indexer/native/) (picked up by the [Open gaps](#open-gaps) glob, so a new driver needs no edit here) |
| **App Sync** | the indexer-create bodies harbrr pushes into the \*arr / qui apps and the per-app decisions for the Servarr-shaped forks (Sonarr/Radarr/Whisparr on `/api/v3`, Lidarr/Readarr on `/api/v1`, qui's native backend), including the version-not-body parameterization, the anime-field omission, and the Readarr-archived / Mylar-deferred dispositions | [`internal/appsync/testdata/README.md`](../internal/appsync/testdata/README.md) |
